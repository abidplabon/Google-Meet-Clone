var AppProcess = (function(){
    ...
    async function videoProcess(newVideoState){
        ...
        if(newVideoState==video_states.Camera){
            $("#videoCamOnOff").html("<span class='material-icons text-success'>present_to_all</span><div>Present Now</div> ");
        }
        try{
            var vstream = null;
            if(newVideoState == video_states.Camera){                      
    ...
    async function setConnection(connid){
        ...
        connection.onicecandidate = function(event){
            if(event.candidate){
                serverProcess(JSON.stringify({icecanditate:event.candidate}),connid)
            }
        }
    ...
    async function setOffer(connid){
        var connection = peers.connection[connid];
    ...
    async function SDPProcess(message,from_connid){
        ...
        }else if(message.icecanditate){
            if(!peers_connection[from_connid]){
                await setConnection(from_connid);
            }
            try{
               await peers_connection[from_connid].addIceCandidate(message.icecanditate); 
            }catch(e){
                console.log(e);
            }
        }
